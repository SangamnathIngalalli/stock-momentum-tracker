...
const SOURCE_FILE = `C:\\Users\\Administrator\\OneDrive\\Swing Trading\\screener\\High ROIC Growth Momentum.xlsx`;
const TODAY_CSV = `C:\\Users\\Administrator\\OneDrive\\check Swing trading\\today_price.csv`;

test('update High ROIC Growth Momentum tracker', async () => {
    ...
    for (let row of existingRows) {
        ...
        if (close !== undefined) {
            ...
            notFoundCount++;
            // If price not found, we still want to maintain the row as is
        }
        updatedRows.push(row);
    }
